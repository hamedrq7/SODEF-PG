...
if do_wandb:
    wandb.init(project="SODEF-MNIST", name=f'MNIST-64D-CenterLoss-FCinit_cent_weight_{exp_name}_no_bias')
...
for itr in trange(5 * batches_per_epoch):
    ...
    if itr % batches_per_epoch == 0:
        ...
        with torch.no_grad():
            ...
            if val_acc > best_acc:
                torch.save({'state_dict': model.state_dict(), 'args': args}, os.path.join(savefolder_fc, 'model_no_bias.pth'))
                best_acc = val_acc
            print(
...
statedict3 = torch.load(os.path.join(savefolder_fc, 'model_no_bias.pth'))['state_dict']
odefunc = ODEfunc_mlp(0)
feature_layers = [ODEBlock(odefunc)]
fc_layers = [MLP_OUT()]
